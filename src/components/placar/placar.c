...
#include "constants/allegro_constants.h" // para SCREEN_WIDTH e SCREEN_HEIGHT

static int pontuacao = 0;
static ALLEGRO_FONT *fonte_placar = NULL;

void inicializar_placar() {
    fonte_placar = al_load_ttf_font("bin/assets/fonts/arial.ttf", 24, 0); // ajuste se necessário
    if (!fonte_placar) {
...
void adicionar_pontos(int linhas_completas) {
    switch (linhas_completas) {
        ...
        default: pontuacao += linhas_completas * 100; break;
    }
}
